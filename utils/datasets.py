...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        pbar = tqdm(zip(self.img_files, self.label_files,repeat(self.cls_names)), desc='Scanning images', total=len(self.img_files))
        for i, (im_file, lb_file,cls_name_list) in enumerate(pbar):
            try:
                # verify images
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        # if any([len(x) > 8 for x in l]):  # Yolov57-obb does not support segment labels yet 
                    ...
                    if len(l):
                        assert len(label) == 10, f'Yolov7-OBB labels require 10 columns, which same as DOTA Dataset, {len(label)} columns detected'
                        assert (l >= 0).all(), 'negative labels'
                        # assert (l[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels'  #YOLOv7-OBB-DOTA 不需要归一化
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w), img_label = load_image(self, index)
        ...
        if self.augment:
            ...
            if random.random() < hyp['fliplr']:
                img = np.fliplr(img)
                if nL:
                    labels[:, 1::2] = img_w - labels[:, 1::2] -1  # [clsid,x1y1 x2y2 x3y3 x4y4]
            # Apply cutouts
        ...
        if nL:
            ...
            ## not already normailzed height and width 0-1
            rboxes = poly2rbox(polys=labels[:, 1:], img_size=(img.shape[0], img.shape[1]), #to use normailze
                                num_cls_thata=hyp['cls_theta'] if hyp else 180, 
                                radius=hyp['csl_radius'] if hyp else 6.0, 
                                use_pi=True, use_gaussian=False)
            labels = np.concatenate((labels[:, :1], rboxes), axis=1)
            
            labels_mask = (rboxes[:, 0] >= 0) & (rboxes[:, 0] < 1) \
                        & (rboxes[:, 1] >= 0) & (rboxes[:, 1] < 1) \
                        & (rboxes[:, 2] > 0.005) | (rboxes[:, 3] > 0.005)# fiter small objects 0.1*1024
            
            labels = labels[labels_mask]
            # labels[:, [2, 4]] /= img.shape[0]  # normalized height 0-1
            # labels[:, [1, 3]] /= img.shape[1]  # normalized width 0-1
            nL = len(labels)  # update after filter
            #  *[clsid poly] to *[clsid cx cy l s theta] θ∈[-pi/2, pi/2) normalized
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            label[:, 1:] *= r  ##because pixel xyxy format so need resize 
...
def load_mosaic(self, index):
    ...
    for x in (segments4):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    labels_mask = poly_filter(polys=labels4[:, 1:].copy(), h= 2 * s, w= 2 * s)
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            ...
            labels[:, [1, 3, 5, 7]] = img_label[:, [1, 3, 5, 7]] + padx
            labels[:, [2, 4, 6, 8]] = img_label[:, [2, 4, 6, 8]] + pady
    ...
    for x in (segments9):
        np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective()
    labels_mask = poly_filter(polys=labels9[:, 1:].copy(), h=2 * s, w=2 * s)
...
def load_samples(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w), image_label = load_image(self, index)
        ...
        # Labels
        labels, segments = image_label.copy(), self.segments[index].copy()
        if labels.size:
            # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            segments = [xyn2xy(x, w, h, padw, padh) for x in segments]
            labels[:, [1, 3, 5, 7]] = image_label[:, [1, 3, 5, 7]] + padw
            labels[:, [2, 4, 6, 8]] = image_label[:, [2, 4, 6, 8]] + padh
        labels4.append(labels)
        segments4.extend(segments)
