...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            tcls.append(c)  # class
            tbox_theta.append(theta) #angle

        return tcls, tbox, tbox_theta, indices, anch
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        Args:
            ...
            self.no = xywh theta conf cls  
            targets (tensor): (n_gt_all_batch, [img_index clsid cx cy l s theta ]) normalized
        ...
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, imgs) # targets many
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True)  # iou(prediction, target) iou not useful
                kldloss = self.kldbbox(pbox_theta,selected_tbox_theta)
                lbox += kldloss.mean()  # iou loss
    ...
    def build_targets(self, p, targets, imgs):
        '''
        方法1 不修改 这部分 直接在后续loss部分换用kld  -- okey
        方法2 ota的cost中iouloss 换成kldloss  --  1. use l1 loss -- 变non   2. use KLD loss  --need 调参  
        '''
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            this_target = targets[b_idx] #  targets (tensor): (n_gt_all_batch, [img_index clsid cx cy l s theta ])
            ...
            # gt_angle_per_image = (
            #     ttheta.float().unsqueeze(1).repeat(1, pxyxys.shape[0], 1)
            # )
            # angle_preds_ = (
            #     p_angles.float().unsqueeze(0).repeat(num_gt, 1, 1)
            # )
            # pair_wise_theta_loss = F.l1_loss(angle_preds_, gt_angle_per_image).sum(-1)
            
            # del angle_preds_
            cost = (
                pair_wise_cls_loss #+ pair_wise_theta_loss
