...
from utils.general import xywh2xyxy, xyxy2xywh,check_requirements 
from utils.metrics import fitness
from utils.rboxs_utils import poly2rbox_new, rbox2poly
...
#             for j, box in enumerate(polys.tolist()):
#                 cls = int(classes[j])
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=2048, max_subplots=4):
    ...
    Args:
        ...
        targets_train (tensor): (n_targets, [batch_id clsid cx cy l s theta ]) θ∈[-pi/2, pi/2)
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            ...
            labels = ti.shape[1] == 7  # labels if no conf column #TODO:
...
def plot_images_val(images, targets, paths=None, fname='images.jpg', names=None, max_size=2048, max_subplots=4):
    """
    Args:
        imgs (tensor): (b, 3, height, width)
        targets_train (tensor): (n_targets, [batch_id clsid cx cy l s theta ]) θ∈[-pi/2, pi/2)
...
def plot_labels(labels, names=(), save_dir=Path(''), img_size=1024,loggers=None):
    rboxes = poly2rbox_new(labels[:, 1:])
...
if __name__ == "__main__":
    plot_results(save_dir='runs/train/base')