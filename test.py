...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
         # targets (tensor): (n_gt_all_batch, [img_index clsid cx cy l s theta]) θ ∈ [-pi/2, pi/2)
        ...
        for si, pred in enumerate(out):
            ...
            path = Path(paths[si])
            shape = shapes[si][0] ## shape (tensor): (h_raw, w_raw)
            seen += 1
            ...
            if save_json: # save hbb pred results and poly pred results.
                ...
                for p, b in zip(pred_polyn.tolist(), box.tolist()):
                    jdict.append({'image_id': image_id,
                                  'category_id': coco91class[int(p[-1])] if is_coco else int(p[-1]+1),
            ...
            correct = torch.zeros(pred_hbbn.shape[0], niou, dtype=torch.bool, device=device)
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    pi = (cls == pred_hbbn[:, 5]).nonzero(as_tuple=False).view(-1)  # target indices
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/dotav2_poly.yaml', help='*.data path')
